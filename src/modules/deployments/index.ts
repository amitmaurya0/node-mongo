...
import Deployments from '../../models/deployments.model';
...
async function getDeployments(req: Request, res: Response){
    try {
        Deployments.find({}, function(err, data){
            if(data){
...
async function createDeployments(req: Request, res: Response){
    try {
        let input = req.body; 
        ...
        let newDeployments = new Deployments({
            name: input.name,
            url: input.url,
...
async function deleteDeployments(req: Request, res: Response){
    try {
        const id = req.params.id;
        Deployments.deleteOne({ id: Object(id) }, function(err){
            if(err){
